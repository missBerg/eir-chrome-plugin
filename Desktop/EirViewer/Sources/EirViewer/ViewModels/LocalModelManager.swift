...
struct LocalModel: Identifiable, Codable, Equatable {
    ...
    var minRAMGB: Int       // Minimum system RAM in GB to run this model
}
...
class LocalModelManager: ObservableObject {
    ...
    static let presets: [LocalModel] = [
        LocalModel(id: "mlx-community/Qwen3.5-0.8B-4bit", displayName: "Qwen 3.5 0.8B (Fast)", minRAMGB: 8),
        LocalModel(id: "mlx-community/Qwen3.5-2B-4bit", displayName: "Qwen 3.5 2B (Balanced)", minRAMGB: 8),
        LocalModel(id: "mlx-community/Qwen3.5-4B-4bit", displayName: "Qwen 3.5 4B (Quality)", minRAMGB: 8),
        LocalModel(id: "mlx-community/Qwen3.5-9B-4bit", displayName: "Qwen 3.5 9B", minRAMGB: 16),
        LocalModel(id: "mlx-community/Qwen3.5-27B-4bit", displayName: "Qwen 3.5 27B", minRAMGB: 32),
    ]
    ...
    /// Whether this model is recommended for the current system's RAM
    static func isRecommended(_ model: LocalModel) -> Bool {
        systemRAMGB >= model.minRAMGB
    }

    // MARK: - Model Management

    func addModel(id: String, displayName: String, minRAMGB: Int = 8) {
        guard !models.contains(where: { $0.id == id }) else { return }
        models.append(LocalModel(id: id, displayName: displayName, minRAMGB: minRAMGB))
    }
    ...
    func loadModel(_ id: String) async {
        ...
        lastErrorModelId = nil
        status = .downloading
        downloadProgress = 0
        DebugLog.log("[LocalModel] Starting download for \(id)")
        ...
        do {
            ...
            DebugLog.log("[LocalModel] Model \(id) loaded, warming up...")
            await service.warmup()
            DebugLog.log("[LocalModel] Model \(id) ready")
            activeModelId = id
            status = .ready
        } catch {
            DebugLog.log("[LocalModel] ERROR loading \(id): \(error)")
            lastErrorModelId = id
    ...
    func modelStatus(_ id: String) -> String {
        ...
        if case .error = status, lastErrorModelId == id {
            return "error"
        }
        return "idle"
    }

    /// The model ID that last failed, so the error row can be shown
    @Published var lastErrorModelId: String?
    ...
    var activeModelDisplayName: String? {
        ...
        return models.first(where: { $0.id == id })?.displayName
    }
